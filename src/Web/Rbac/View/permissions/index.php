                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($permissions as $perm): ?>
                            <tr>
                                <td><code class="code-badge"><?= Html::encode($perm['name']) ?></code></td>
                                <td class="text-muted text-sm"><?= Html::encode($perm['description'] ?? '-') ?></td>
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <form action="<?= $urlGenerator->generate('permissions/delete', ['name' => $perm['name']]) ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus izin (permission) ini? Menghapus izin ini juga akan mencopot hak ini dari semua peran yang dikaitkan.');" class="form-inline">
                                            <input type="hidden" name="_csrf" value="<?= Html::encode($csrf) ?>">
                                            <button type="submit" class="btn-action btn-delete <?= $perm['name'] === 'manage_rbac' ? 'opacity-50 cursor-not-allowed' : '' ?>" title="Hapus Permission" <?= $perm['name'] === 'manage_rbac' ? 'disabled' : '' ?>>
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
