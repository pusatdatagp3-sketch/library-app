    ...
    <form action="<?= $formAction ?>" method="POST" class="form-grid">
        ...
        <div class="form-group">
            <?php if ($isUpdate): ?>
                ...
                <input type="text" class="form-control form-control-disabled" value="<?= Html::encode($permission['name']) ?>" disabled>
                <small class="form-hint">Kunci hak akses unik dan tidak dapat diubah setelah didaftarkan.</small>
            <?php else: ?>
                ...
                <small class="form-hint">Disarankan menggunakan gaya snake_case. Hanya huruf, angka, tanda hubung (-), dan garis bawah (_).</small>
            <?php endif; ?>
        </div>

        <div class="form-group">
            <label for="description" class="form-label">Penjelasan / Deskripsi Kegunaan</label>
            <textarea id="description" name="description" class="form-control" placeholder="Contoh: Mengizinkan pengguna untuk membuka dan mengunduh laporan keuangan bulanan." rows="4"<?= $isUpdate ? ' required' : '' ?>><?= Html::encode($data['description']) ?></textarea>
        </div>
