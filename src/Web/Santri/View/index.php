...
/**
 * @var WebView $this
 * @var App\Web\Santri\Model\Santri[] $models
...
$this->setTitle('Data Santri');
?>

<div class="crud-container">
    <div class="crud-header">
        <div>
            <h1 class="crud-title">Data Santri</h1>
            <p class="crud-subtitle">Kelola informasi data santri di sistem.</p>
        </div>
        
        <?php if ($userSession->hasPermission('create_santri')): ?>
            <a href="<?= $urlGenerator->generate('santri/create') ?>" class="btn btn-primary">
    ...
    <?php else: ?>
        <div class="table-responsive card">
            <table class="table">
                <thead>
                    <tr>
                        <th style="width: 80px;">KDS</th>
                        <th>Nama</th>
                        <th>Kelas</th>
                        <th>Daerah</th>
                        <?php if ($userSession->hasPermission('update_santri') || $userSession->hasPermission('delete_santri')): ?>
                ...
                <tbody>
                    <?php foreach ($models as $model): ?>
                        <tr>
                            <td><span class="badge badge-id"><?= Html::encode((string)$model->kds) ?></span></td>
                            <td><?= Html::encode((string)$model->nama) ?></td>
                            <td><?= Html::encode((string)$model->kelas) ?></td>
                            <td><?= Html::encode((string)$model->daerah) ?></td>
                            <?php if ($userSession->hasPermission('update_santri') || $userSession->hasPermission('delete_santri')): ?>
                                <td>
                                    <div class="action-buttons">
                                        <?php if ($userSession->hasPermission('update_santri')): ?>
                                            <a href="<?= $urlGenerator->generate('santri/update', ['id' => $model->kds]) ?>" class="btn-action btn-edit">
                                                Edit
                                            </a>
                                        <?php endif; ?>

                                        <?php if ($userSession->hasPermission('delete_santri')): ?>
                                            <form action="<?= $urlGenerator->generate('santri/delete', ['id' => $model->kds]) ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');" style="display:inline;">
