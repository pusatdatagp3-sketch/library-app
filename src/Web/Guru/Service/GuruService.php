...
namespace App\Web\Guru\Service;

use App\Web\Guru\Model\GuruEntity;
...
final class GuruService
{
    ...
    public function updateGuru(int $kdg, array $rawData, array &$errors): bool
    {
        ...
        $entity = $this->guruRepository->getById($kdg);
        if ($entity === null) {
            $errors['general'] = 'Data guru tidak ditemukan.';
            return false;
        }
    ...
    public function importExcel(string $filePath, array &$errors): int
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($stambuk === '') { $errors[] = "Baris {$rowNum}: Stambuk tidak boleh kosong."; continue; }
            if ($nama === '')    { $errors[] = "Baris {$rowNum}: Nama tidak boleh kosong.";    continue; }
    ...
    private function validateDto(GuruDto $dto): array
    {
        ...
        if ($dto->noTelp === '')   { $errors['no_telp']  = 'Nomor telefon tidak boleh kosong.'; }
        return $errors;
    }
}
